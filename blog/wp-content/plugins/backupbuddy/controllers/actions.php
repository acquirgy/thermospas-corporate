		...
		if ( is_array( pb_backupbuddy::$options['schedules'][$cron_id] ) ) {
			...
			foreach( $destinations as $destination ) {
				if ( isset( $destination ) && ( $destination != '' ) ) {
					if ( isset( pb_backupbuddy::$options['remote_destinations'][$destination] ) ) { // Destination ID is valid; destination hasnt been deleted or anything.
						pb_backupbuddy::status( 'details', 'Adding send step.' );
			...
			if ( true === $found_valid_destination ) { // At least one destination is set. Prevents deleting local backup if no destination set.
				if ( pb_backupbuddy::$options['schedules'][$cron_id]['delete_after'] == '1' ) {
					pb_backupbuddy::status( 'details', 'Option to delete local file after remote send enabled; adding step.' );
					array_push( $post_backup_steps, array(
			...
			} else { // No destinations but delete_after was set. Unset it & report.
				pb_backupbuddy::$options['schedules'][$cron_id]['delete_after'] = 0;
				pb_backupbuddy::status( 'message', 'Schedule set to delete local file after remote send but no destinations exist. Unsetting option.' );
				pb_backupbuddy::save();
			}
