	...
	if ( pb_backupbuddy::$options['backup_directory'] != $backup_directory ) { // Directory differs. Needs updated in post var. Give messages here as this value is going to end up being saved.
		...
		foreach( $old_backups as $old_backup ) {
			if ( false === rename( $old_backup, $new_backup_dir . basename( $old_backup ) ) ) {
				...
				if ( isset( pb_backupbuddy::$options['backups'][ $serial ] ) ) {
					if ( isset( pb_backupbuddy::$options['backups'][ $serial ]['archive_file'] ) ) {
						pb_backupbuddy::$options['backups'][ $serial ]['archive_file'] = str_replace( $old_backup_dir, $new_backup_dir, pb_backupbuddy::$options['backups'][ $serial ]['archive_file'] );
					}
				}
			}
		}
...
$settings_form->add_setting( array(
	...
	'tip'		=>		__( '[Default: Disabled] When enabled, WordPress will skip local https SSL verification.', 'it-l10n-backupbuddy' ) . '</span>',
	'css'		=>		'',
