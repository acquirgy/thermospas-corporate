...
class pb_backupbuddy_updater {
	...
	/* __construct()
	 *
	 * Default constructor that auto-runs on instantiation.
	 *
	 */
	...
	/* init()
	 *
	 * Function called by WordPress on admin init. Get this party started.
	 *
	 */
	public function init() {
		if ( current_user_can( 'activate_plugins' ) ) {
			if ( isset( $_GET[ 'pluginbuddy_refresh' ] ) ) {
				$this->check_for_updates( true ); // Force check.
			} else {
...
//TODO		//$this->plugins = $this->get_plugin_options();
	...
	public function perform_remote_request( $args ) {
		...
		$username = '';
		if ( isset( $this->plugins[ 'username' ] ) ) {
