...
class pb_backupbuddy_core {
		...
		if ( is_array( $files ) && !empty( $files ) ) { // For robustness. Without open_basedir the glob() function returns an empty array for no match. With open_basedir in effect the glob() function returns a boolean false for no match.
			...
			foreach( $files as $file_id => $file ) {
				...
				if ( isset( pb_backupbuddy::$options['backups'][$serial]['integrity']['comment'] ) && ( pb_backupbuddy::$options['backups'][$serial]['integrity']['comment'] !== false ) && ( pb_backupbuddy::$options['backups'][$serial]['integrity']['comment'] !== '' ) ) {
					$main_string .= '<br><span class="description">Note: <span class="pb_backupbuddy_notetext">' . htmlentities( pb_backupbuddy::$options['backups'][$serial]['integrity']['comment'] ) . '</span></span>';
				}
	...
	public function preflight_check() {
		...
		if ( is_multisite() && pb_backupbuddy::$classes['core']->is_network_activated() && !defined( 'PB_DEMO_MODE' ) ) { // Multisite installation.
			$tests[] = array(
				'test'		=>	'multisite_beta',
				'success'	=>	false,
				'message'	=>	'WARNING: BackupBuddy Multisite functionality is not supported. Multiple issues are known. Usage of it is at your own risk and should not be relied upon. Standalone WordPress sites are suggested. You may use the "Export" feature to export your subsites into standalone WordPress sites. To enable experimental BackupBuddy Multisite functionality you must add the following line to your wp-config.php file: <b>define( \'PB_BACKUPBUDDY_MULTISITE_EXPERIMENT\', true );</b>
								'
			);
			/*
			$tests[] = array(
		...
		if ( ! is_multisite() ) {
			...
			$found_plugins = array();
			foreach( $warn_plugins as $warn_plugin => $warn_plugin_title ) {
	...
	function periodic_cleanup( $backup_age_limit = 43200, $die_on_fail = true ) {
		...
		// Check for orphaned backups in the data structure that havent been updated in 12+ hours & cleanup after them.
