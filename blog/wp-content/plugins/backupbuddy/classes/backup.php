...
class pb_backupbuddy_backup {
	...
	function process_backup( $serial, $trigger = 'manual' ) {
		...
		if ( method_exists( $this, $step['function'] ) ) {
			...
			pb_backupbuddy::status( 'details', 'Starting step function `' . $step['function'] . '` with args `' . $args . '`.' );
		...
		if ( $response === false ) { // Function finished but reported failure.
			...
			pb_backupbuddy::status( 'details', sprintf( __('Finished function `%s`. Peak memory usage', 'it-l10n-backupbuddy' ) . ': ' . round( memory_get_peak_usage() / 1048576, 3 ) . ' MB', pb_backupbuddy::$options['backups'][$serial]['steps'][$step_index]['function'] ) );
			pb_backupbuddy::status( 'details', '-----' );
