...
if ( !class_exists( "pluginbuddy_zbzipproc" ) ) {
	...
	class pluginbuddy_zbzipproc extends pluginbuddy_zbzipcore {
		...
		protected function set_unzip_version( $major = 0, $minor = 0, $override = false ) {
			...
			// See if we can find value - we know unzip is available so just press on...
			// Determine if we are using an absolute path
			if ( isset( $this->_method_details[ 'param' ][ 'path' ] ) ) {
		...
		public function create( $zip, $dir, $compression, $excludes, $tempdir, $listmaker = NULL ) {
			...
			switch ( $this->get_os_type() ) {
				case self::OS_TYPE_NIX:
					$result = $this->create_linux( $zip, $dir, $compression, $excludes, $tempdir, $listmaker );
					break;
				case self::OS_TYPE_WIN:
					$result = $this->create_windows( $zip, $dir, $compression, $excludes, $tempdir, $listmaker );
					break;
				default:
					$result = false;
			}
