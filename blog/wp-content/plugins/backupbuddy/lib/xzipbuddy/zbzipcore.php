...
if ( !class_exists( "pluginbuddy_zbzipcore" ) ) {
	...
	abstract class pluginbuddy_zbzipcore {
		...
		const MAX_WARNINGS_LINES_TO_SHOW = 20;
		const NORM_DIRECTORY_SEPARATOR = '/';
		const DIRECTORY_SEPARATORS = '/\\';
		
		// Enumerated OS types that we need for now
		...
		public static $_warning_desc = array( self::ZIP_WARNING_UNKNOWN  => 'warning reason unknown',
											  ...
											  self::ZIP_WARNING_SKIPPED  => 'unreadable or does not exist',
											  self::ZIP_WARNING_FILTERED => 'filtered',
		...
		public function __construct() {
			...
			$this->_method_details[ 'param' ] = array( 'path' => '',
													   'zip' => array( 'version' => array( 'major' => 0, 'minor' => 0 ), 'options' => '', 'info' => '' ),
													   'unzip' => array( 'version' => array( 'major' => 0, 'minor' => 0 ), 'options' => '', 'info' => '' )
													 );
