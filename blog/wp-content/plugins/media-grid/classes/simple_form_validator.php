...
class simple_fv {
	...
	private function validate($type, $val, $index_val, $label, $test=false) {
		...
		if($type == 'min_len') {
			if(strlen($index_val) < (int)$val) {
				if(!$test) {
					...
					$this->custom_error[$label] = _('minimum').' '.$val.' '._('characters allowed');
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'max_len') {
			if(strlen($index_val) > (int)$val) {
				if(!$test) {
					#$this->errors['lenght'][] = $label;
					$this->custom_error[$label] = _('maximum').' '.$val.' '._('characters allowed');
				}
				return false;
			}	
			else {return true;}
	...
	private function errorTranslate($type, $label_array) {
		...
		switch($type) {
			case 'required' : 
				(count($label_array) > 1) ? $message .= _('are required') : $message .= _('is required');
				break;
			...
			case 'not_equal' : $message .= ("the value doesn't match"); 
				break;
