...
1. enableBlurb
2. enableUnlimitedSidebars
3. enableCustomThumbnails
4. controlBackground
5. customFormatter
6. megaMenu
...
class Enchance
{
	...
	public function enableUnlimitedSidebars()
	{
		function ulsidebars_add_custom_box() {
			 ...
			 add_meta_box('ulsidebars', 'Sidebars', 'ulsidebars_custom_box','gallery', 'side', 'high');
		}
		...
		function ulsidebars_custom_box() {
			 ...
			 $sidebar = get_post_meta($post->ID,'_dynamic_sidebar',true)  ;
			  ...
			  $active_sidebars = get_option("hades_active_sidebars");
	...
	public function controlBackground()
	{
		...
		if(get_option("hades_bg_color_toggle")!="false")	
		$color = (!get_option("hades_bg_color")) ? "#ffffff" :  '#'.get_option("hades_bg_color");
		
		 $texture =  get_option('hades_bg_texture');
	  ...
	  if(!get_option("hades_bg_image_scale")||get_option("hades_bg_image_scale")=="true")
	  ...
	  $fcolor = get_option("hades_font_color");
