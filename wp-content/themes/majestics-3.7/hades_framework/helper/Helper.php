							        ...
							        if($show_tags)
									  {
										  ...
										  foreach($tags as $tag)
										  {
											  $ptag .= "<a href='".get_tag_link($tag->term_id)."'>".__($tag->name,'h-framework')."</a>, ";
										  }
									  }
									  echo $ptag;
							?>
		...
		if ( !is_home() && !is_front_page() || is_paged() ) {
		  ...
		  if ( is_category() ) {
			...
			if ( get_post_type() != 'post' ) {
			  ...
			  echo '<a href="' . $homeLink . '/' . $slug['slug'] . '/">' . __($post_type->labels->singular_name ,'h-framework'). '</a> ' . $delimiter . ' ';
			...
			while ($parent_id) {
			  ...
			  $breadcrumbs[] = '<a href="' . get_permalink($page->ID) . '">' . __( get_the_title($page->ID) ,'h-framework'). '</a>';
