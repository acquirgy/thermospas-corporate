...
<?php if(isset($_GET["stage"])) : ?>
  ...
  //   sethomecolumns();
   ...
   <!-- ===================================================================================================== -->
    ...
    $rows = $wpdb->get_results("SELECT * FROM $table_db_name where name='Top Menu' OR name='Main Menu' OR name='Footer Menu'",ARRAY_A);
    $menu_ids = array();
	foreach($rows as $row)
...
<?php if($_GET["stage"]=="titan_templates") : 
 ...
 install_templates(); 
   ...
   $gmes = "Slider ";
    update_option( 'hades_slides' ,  $h_slides  );
...
<div id="starter-kit" class="clearfix">
 ...
 <h4>Majestics is using TimThumb to resize the images.</h4>
...
<li>Don't look at the front-end until all steps are finished, this to avoid confusing.</li>
  ...
  <ul class="helper-tree" >
   <li class="clearfix  <?php if(get_option("titan_install_done")) echo "importer_"; ?>">
   ...
   <li class="clearfix <?php if(!get_option("titan_install_done")) echo ""; ?>">
   ...
   <li class="clearfix <?php if(!get_option("titan_install_done")){   echo ""; } if(get_option("titan_menus")=="yes") echo "disable_menu"; ?>">
   ...
   <li class="clearfix <?php if(!get_option("titan_install_done")) echo ""; ?>">
      <h4>Activate Titan Templates</h4>  
        
        <div class="formatted">
   
        
        <h5>What will happen after i've activated the Titan Templates</h5>
        <p>This options was made to import and populate all TitanEditor templates. Shortcodes, columns, buttons, tabs, toggles and any other shortcode pages that's shown inside the demo set up.</p>

      
      
      <div class="highlight2">   
       
     <p><a href="<?php echo $url."&stage=titan_templates"; ?>" class="tbutton">Click here to activate the Titan Templates</a></p>
     </div>
     </div>
     </li>
