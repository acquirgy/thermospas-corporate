...
if (!empty($images)) {
	foreach ($images as $image) {
		...
		<div class="image_wrap">
			...
			<div class="fgallery_image">
			<?php 				
				echo '<img src="'.EXTRA_DIR.$image->path.'/thumbs/thumbs_'.$image->filename.'" alt="'.$image->alttext.'" />';
			?>
 ...
 function fgallery_edit_album_images_page($id){
	...
	$album = fgallery_get_album($id);
		...
		echo '<h2>'.__('Images in', 'fgallery').'<a href="'.fgallery_get_edit_url($id).'"> '.$album['gall_name'].'</a> 
                    '.__('gallery','fgallery').'</h2><div class="tablenav">';
...
function fgallery_edit_image_page($id){
	...
	if ($image['img_vs_folder'] == 0) {
		...
		echo '</div><a href="'.get_option('siteurl').'/'.$image['img_path'].'" title="'.__('Full view','fgallery').'" target="_blank">
...
function fgallery_get_nextgen_images() {
	global $wpdb;
		$images = $wpdb->get_results("SELECT a.pid, a.description, a.alttext, a.filename, b.path FROM ".$wpdb->prefix."ngg_pictures as a LEFT JOIN ".$wpdb->prefix."ngg_gallery as b ON (a.galleryid = b.gid)");
	return $images;
}
...
function sc_params_pane($album) {
  ...
  if (FGALLERY_PHP4_MODE) {
	...
	// "Save" button
  $form .= '<input name="sc_submit" id="save_gallery_settings" type="submit" value="'.__('Save').'" /><span id="configurator_message"></span>';
		...
		$output .= '<li id="image_'.$item['img_id'].'" class="'.$cover_class.'"><div class="image_wrap">
                ...
                if ($item['img_vs_folder'] == 0){		
                        $output .= '<div class="edit_gallery_urls">
                            <img src="'.COVER_PATH.$item['img_path'].'" alt="'.$item['img_caption'].'" /><br /><br />
                ...
                <div class="fgallery_image_actions">
                        ...
                        if ($item['img_vs_folder'] != 2) {
                            ...
                            $output .= '<a href="'.fgallery_get_addimages_url($gall_id, $item['img_id']).'" class="thickbox" title="'.$title_4.'">'.$title_4.'</a>';
                        }
                $output .='</div></div></li>';
	}
        $output .= '<a class="save_all_album_images" href="javascript:void(0)">'.__('Save All Changes').'</a>';
...
function fgallery_get_addimages_url($id, $folder_id = 0) {
	return FGALLERY_PATH.'/addimage.php?id='.$id.'&fid='.$folder_id.'TB_iframe=1';
}
