...
function fgallery_render_addimage_box($items, $id) {
	...
	$output = '<h2>'.__('Adding Images to ', 'fgallery').'"'.fgallery_get_album_name($id).'"'.$album_name.'</h2>';
        if (isset($_GET['folder']) && is_numeric($_GET['folder'])) {
            $output .= '<p>'.__('You are here : ', 'fgallery').
                    '<a href="'.fgallery_get_addimages_url($id).'">'.__('All images', 'fgallery').'...</a>->'
	...
	if (isset($_GET['folder'])) {
		$output .=  '<a class="go_up" href="'.fgallery_get_addimages_url($id).'">'.__('Go up', 'fgallery').'...</a>';
	}
	$output .= '<ul>';
		...
		$output .= '</ul><input type="hidden" value="'.$id.'" name="gall_id" />
		...
		if (count($ids) > 0) {
			foreach ($ids as $id) {
				...
				if ($image['img_vs_folder'] == 1) {
					...
					$wpdb->insert(IMAGES_TO_ALBUMS_TABLE, array('img_id'=>$id, 'gall_id'=>$gall_id, 'img_order' => 0, 'gall_folder' => $fid));
				}
			}
		}
		session_unset();
