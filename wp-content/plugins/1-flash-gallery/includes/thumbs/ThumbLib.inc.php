...
class PhpThumbFactory
{
	 ...
	 */
	public static function create($filename = null, $options = array(), $isDataStream = false, $fullpath = 0)
	{
                if ($fullpath) {
                    $filename = THUMBLIB_ABSPATH.$filename;
                }
		...
		// load the plugins
		$pt->loadPlugins(THUMBLIB_PLUGIN_PATH);
		
		$toReturn = null;
		$implementation = DEFAULT_THUMBLIB_IMPLEMENTATION;
		
		// attempt to load the default implementation
		if ($pt->isValidImplementation(DEFAULT_THUMBLIB_IMPLEMENTATION))
		{
			$imp = $implementationMap[DEFAULT_THUMBLIB_IMPLEMENTATION];
