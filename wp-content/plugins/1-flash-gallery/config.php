...
if (isset($_GET['gall_id']) && is_numeric($_GET['gall_id'])) {
		...
		echo '<gallery>
				...
				<type>byFolder</type>
                        ...
                        <slideshow>
                                ...
                                <fullscreenEnable>'.(int)$config['sc_slideshow__fullscreenEnable'].'</fullscreenEnable>';
				echo '<'.get_option('1_flash_gallery_pax', MUSIC_PATH).'>'
				.$config['sc_slideshow__musicPath'].
				'</'.get_option('1_flash_gallery_pax', MUSIC_PATH).'>';
                           echo '<backgroundType>'.$config['sc_slideshow__backgroundType'].'</backgroundType>
                                ...
                                <theme>'.$config['sc_slideshow__theme'].'</theme>
                                <target>'.$config['sc_slideshow__target'].'</target>
                        </slideshow>
                        <scroller>
                                <fogEnable>'.$config['sc_scroller__fogEnable'].'</fogEnable>
                                <fogWidth>'.$config['sc_scroller__fogWidth'].'</fogWidth>
                                <xItemDistance>'.$config['sc_scroller__xItemDistance'].'</xItemDistance>
                                <yItemDistance>'.$config['sc_scroller__yItemDistance'].'</yItemDistance>
                                <scrollingDirection>'.$config['sc_scroller__scrollingDirection'].'</scrollingDirection>
                                <scrollingSpeed>'.$config['sc_scroller__scrollingSpeed'].'</scrollingSpeed>
                        </scroller>
                        <item>
                                <width>'.$config['sc_item__width'].'</width>
                                <height>'.$config['sc_item__height'].'</height>
                                <scale>'.$config['sc_item__scale'].'</scale>
                                <cornerRadius>'.$config['sc_item__cornerRadius'].'</cornerRadius>
                                <borderThickness>'.$config['sc_item__borderThickness'].'</borderThickness>
                                <borderColor>'.$config['sc_item__borderColor'].'</borderColor>
                                <titlePosition>'.$config['sc_item__titlePosition'].'</titlePosition>
                                <titleAlign>'.$config['sc_item__titleAlign'].'</titleAlign>
                                <titleFont>'.$config['sc_item__titleFont'].'</titleFont>
                                <titleSize>'.$config['sc_item__titleSize'].'</titleSize>
                                <titleFontColor>0x'.str_replace('#','',$config['sc_item__titleFontColor']).'</titleFontColor>
                                <titleBackgroundColor>0x'.str_replace('#','',$config['sc_item__titleBackgroundColor']).'</titleBackgroundColor>
                                <titleBold>'.$config['sc_item__titleBold'].'</titleBold>
                                <shadowDistance>'.$config['sc_item__shadowDistance'].'</shadowDistance>
                                <shadowSize>'.$config['sc_item__shadowSize'].'</shadowSize>
                                <shadowColor>0x'.str_replace('#','',$config['sc_item__shadowColor']).'</shadowColor>
                                <shadowAngle>'.$config['sc_item__shadowAngle'].'</shadowAngle>
                                <selectTintEnable>'.$config['sc_item__selectTintEnable'].'</selectTintEnable>
                                <preloaderEnable>'.$config['sc_item__preloaderEnable'].'</preloaderEnable>
                                <preloaderColor>0x'.str_replace('#','',$config['sc_item__preloaderColor']).'</preloaderColor>
                        </item>
                        <preview>
                                <width>'.$config['sc_preview__width'].'</width>
                                <height>'.$config['sc_preview__height'].'</height>
                                <scale>'.$config['sc_preview__scale'].'</scale>
                                <captionPosition>'.$config['sc_preview__captionPosition'].'</captionPosition>
                                <captionAlign>'.$config['sc_preview__captionAlign'].'</captionAlign>
                                ...
                                <captionBackgroundColor>0x'.str_replace('#','',$config['sc_preview__captionBackgroundColor']).'</captionBackgroundColor>
                                <dimmingBackgroundEnable>'.$config['sc_preview__dimmingBackgroundEnable'].'</dimmingBackgroundEnable>
                                <linkageEnable>'.$config['sc_preview__linkageEnable'].'</linkageEnable>
                                <buttonsEnable>'.$config['sc_preview__buttonsEnable'].'</buttonsEnable>
                        </preview>
                        <navigation>
                                <enable>'.$config['sc_navigation__enable'].'</enable>
                                <align>'.$config['sc_navigation__align'].'</align>
                                <name>'.$config['sc_navigation__name'].'</name>
                                <colorButton>0x'.str_replace('#','',$config['sc_navigation__colorButton']).'</colorButton>
                                <colorBackground>0x'.str_replace('#','',$config['sc_navigation__colorBackground']).'</colorBackground>
                                <albumIconEnable>'.$config['sc_navigation__albumIconEnable'].'</albumIconEnable>
                        </navigation>';
	break;
	...
	case 7:
		...
		echo '<main>
				...
				<flipAlign>'.$config['sc_main__flipAlign'].'</flipAlign>
